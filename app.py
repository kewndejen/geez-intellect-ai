...
st.markdown("""
    ...
    .wait-msg { color: #FFD700; font-style: italic; font-size: 1rem; text-align: center; font-weight: bold; background: rgba(0,0,0,0.4); padding: 15px; border-radius: 10px; border: 1px dashed #FFD700; }
...
# ---------------------------------------------------------
# 2. DOCUMENT ARCHIVE LOGIC (PDF/Word)
# ---------------------------------------------------------
...
# ---------------------------------------------------------
# 3. INDESTRUCTIBLE ENGINE (Retry & Discovery Logic)
# ---------------------------------------------------------
...
def discover_stable_engines():
    try:
        ...
        priority = ["models/gemini-1.5-flash", "models/gemini-1.5-pro", "models/gemini-pro", "models/gemini-2.0-flash-exp"]
...
def ask_sovereign_scholar(prompt, tool_context, document_archive=""):
    ...
    # ተስፋ የማይቆርጥ ሎጂክ (Retry Loop across all models)
    for model_name in ACTIVE_MODELS:
        try:
            model = genai.GenerativeModel(model_name=model_name, system_instruction=sys_instr)
            
            # ለእያንዳንዱ ሞዴል 3 ጊዜ በራሱ ይሞክራል (Exponential Backoff)
...
# ---------------------------------------------------------
# 4. SIDEBAR: THE ARK OF 60 PILLARS (The Complete Ark)
# ---------------------------------------------------------
with st.sidebar:
    ...
    # 📂 Gemini 3 Style Multi-Doc Vault
...
# ---------------------------------------------------------
# 5. MAIN WORKSPACE (The Master Interface)
# ---------------------------------------------------------
...
if prompt := st.chat_input(f"Consult the {tool} expert..."):
    ...
    with st.chat_message("assistant"):
        with st.spinner("ሊቁ መዛግብቱን በጥልቀት እያመሳከረ ነው..."):
            ...
            <div class='citation'>Source: {engine} | Masterpiece Sovereign v6.0</div>
            """
