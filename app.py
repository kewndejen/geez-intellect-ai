...
# ---------------------------------------------------------
st.set_page_config(
    page_title="GE'EZ STUDIO | Sovereign Shield",
    page_icon="🔱",
    layout="wide",
    initial_sidebar_state="expanded"
)

# Majestic Emerald Green & Royal Gold Theme (Extreme Contrast)
st.markdown("""
    <style>
    ...
    .wait-msg { color: #FFD700; font-style: italic; font-size: 1.1rem; text-align: center; font-weight: bold; background: rgba(0,0,0,0.5); padding: 15px; border-radius: 15px; border: 1px dashed #FFD700; }
    </style>
...
@st.cache_resource
def get_stable_engines():
    try:
        available = [m.name for m in genai.list_models() if 'generateContent' in m.supported_generation_methods]
        priority = ["models/gemini-1.5-flash", "models/gemini-1.5-flash-8b", "models/gemini-1.5-pro", "models/gemini-pro"]
        return [p for p in priority if p in available] or [available[0]]
    except: return ["models/gemini-1.5-flash"]

ACTIVE_MODELS = get_stable_engines()

def ask_sovereign_scholar(prompt, tool_context, document_archive=""):
    sys_instr = f"You are 'Ge'ez Scholar AI Master', created by Grand Architect Deacon Kewn Dejen. Expert in {tool_context}. Primary Source: {document_archive[:20000]}"
        ...
        try:
            ...
            for attempt in range(1, 4):
                try:
                    ...
                    if "429" in str(e):
                        wait = (attempt * 10) # 10, 20 ሰከንድ ቆም ይላል
                        status_placeholder.markdown(f"<div class='wait-msg'>⏳ ሊቁ ጥልቅ ምርምር ላይ ናቸው... እባክዎ ጥቂት ሰከንዶች ይጠብቁ (ሙከራ {attempt}/3)</div>", unsafe_allow_html=True)
                        time.sleep(wait)
                        continue
                    break
        except: continue
...
with st.sidebar:
    ...
    st.markdown(f"<div style='background:linear-gradient(45deg, #FFD700, #B8860B); padding:15px; border-radius:12px; text-align:center; color:#000; font-weight:900;'>GRAND ARCHITECT: DEJ. KEWN DEJEN</div>", unsafe_allow_html=True)
    st.markdown("---")
    
    # Document Vault
    st.subheader("📂 የሰነድ መዝገብ (Vault)")
    uploaded_files = st.file_uploader("PDF/Word ፋይሎችን እዚህ ይደረድሩ", type=['pdf', 'docx'], accept_multiple_files=True)
    
    if "global_memory" not in st.session_state: st.session_state.global_memory = ""
    if uploaded_files:
        combined = ""
        for f in uploaded_files: combined += f"\n[FILE: {f.name}]\n" + extract_text(f)
        st.session_state.global_memory = combined
    ...
    tools_map = {
        "Advanced AI Labs": ["Manuscript OCR", "Linguistic Bridge", "Script Authentication", "Root Finder", "Voice of Wisdom", "Syntax Analyzer", "Ge'ez NLP", "Dialect Study", "Semantic Map", "Neural Translation"],
        "Archives & Law": ["Universal Library", "Fetha Nagast AI", "Synaxarium Analysis", "Royal Decrees", "Treaty Expert", "Kibre Nagast Hub", "Ecclesiastical Law", "Genealogy Map", "Preservation Lab", "Hagiography Lab"],
        ...
        "University Hub": ["Bahre Hasab Pro", "Abu Shaker Astronomy", "Numerology Lab", "Scribe Assistant", "Font Converter", "Ethiopic Math", "Philosophy Hub", "History Chronology", "University Portal", "Scholarly Citation"],
...
if prompt := st.chat_input("ለሊቁ ጥያቄዎን እዚህ ያስገቡ..."):
    ...
    with st.chat_message("assistant"):
        with st.spinner("ሊቁ መዛግብቱን በጥልቀት እያመሳከረ ነው..."):
            ...
            full_res = f"<div>{answer}</div><div style='font-size:0.8rem; color:#FFD700; margin-top:20px; border-top:1px solid #ffffff33; padding-top:10px;'>Source: {engine} | v5.0 Sovereign Shield</div>"
